      ...
      <ScrollView style={[styles.mainView]}>
        <View style={styles.cardView}>
          ...
          <View style={styles.body}>
            ...
            <Button disabled={!isEnabled} style={{ opacity: isEnabled ? 1 : 0.5, marginTop: signUp ? 20 : 80, backgroundColor: Colors.middleBlue, width: '90%', height: 55, borderRadius: 8, flexDirection: 'row', alignItems: 'center' }} onPress={() => this.signTrigger()}>
              {loading
                ? <ActivityIndicator style={{ alignItems: 'center', flex: 1, }} size='large' color='white' />
                : <View style={{ flexDirection: 'row', alignItems: 'center', }}>
             
                  <Text style={{ fontSize: 20, color: Colors.white, flex: 1, textAlign: 'center' }}>{buttonValue}</Text>
                </View>}
            </Button>

            <Button style={{marginVertical:20,height:50}} onPress={() => this.toggleSignInScreens()}>
              <Text style={{ fontSize: 15, color: Colors.midLightGrey, marginHorizontal: 20, paddingTop: 10,paddingBottom:10 }}>{textValue}</Text>
            </Button>
