    ...
    const isFocused = useIsFocused()
      return (
        <View style={[styles.renderView, { flex: 1, overflow: 'hidden' }]}>
          ...
          {/* <Video isMuted={this?.state?.isMuted} paused={pauseVideo || (currentIndex !== index)} source={{
            // uri: `https://a-p-e.herokuapp.com/user/reels/watch?uuid=8c32993b-02e5-499c-8a40-b075b3e251e4&access_token=${authStore.token}`,
            uri: item.video,
            type: 'mp4',
            headers: {
              'range': 'bytes=0-10000',
          ...
          {/* <Video isMuted={this?.state?.isMuted} paused={pauseVideo || (currentIndex !== index)} source={item.video}
            setRef={(ref) => { this.video = ref }} /> */}
          <TouchableOpacity style={styles.videoTitleView} onPress={() => data && navigation?.navigate('StoreDetailsScreen', { data })}>
  
            <View style={{ backgroundColor: 'white', borderRadius: 10 }}>
  
              {/* <Image source={{ uri: item?.metadata?.branch?.image }} style={styles.companyLogoImage} /></View> */}
              {/* <Image source={{ uri: item?.metadata?.branch?.image }} style={styles.companyLogoImage} /></View> */}
